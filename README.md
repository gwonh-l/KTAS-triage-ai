...
| 단계 | 구성요소 | 저장소 / 위치 | 설명 |
|---|---|---|---|
| ① 데이터 생성 | ER Data Generator | `ER_Data_Generator/` (별도) | KTAS 분류표 기반 가상 증상 텍스트를 GPT로 생성·정제·증강 |
...
> 데이터셋은 GPT로 생성한 **가상 데이터**이며 실제 환자 정보(PHI)를 포함하지 않습니다. 생성 파이프라인 상세는 `ER_Data_Generator/README.md`를 참고하세요.
...
> 데이터 생성 파이프라인(GPT 증상 텍스트 생성)은 별도 저장소 `ER_Data_Generator/`에 있습니다.
...
- **데이터 자체 신뢰도 문제** — KTAS 분류 기준에 따른 GPT 생성 텍스트를 기반으로 하기에 신뢰도에 문제가 있습니다.
